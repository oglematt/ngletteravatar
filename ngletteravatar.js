 ...
 * @version v4.0.1 - 2015-09-28 * @link https://github.com/uttesh/ngletteravatar
 * @author Uttesh Kumar T.H. <uttesh@gmail.com>
 * @fork Matt Ogle <mogle@eiase.com>
...
nla.constant('defaultSettings', {
	...
	fontSize: 35, // unit in pixels
	height: 80, // unit in pixels
	width: 80, // unit in pixels
	fontWeight: 400, //
	charCount: 2,
	fontFamily: 'Roboto Bold, HelveticaNeue-Light,Helvetica Neue Light,Helvetica Neue,Helvetica, Arial,Lucida Grande, sans-serif',
	...
	return {
		...
		link: function(scope, element, attrs) {
			...
			var params = {
				...
				shape: isNotNull(attrs.shape) ? attrs.shape : defaultSettings.shape,
				radius: isNotNull(attrs.height) ? attrs.height : defaultSettings.height
			};

			if (isNotNull(attrs.fontSize)) {
				params.fontSize = attrs.fontSize;
			} else if (isNotNull(attrs.height)) {
				params.fontSize = attrs.height * .375
			} else {
				params.fontSize = defaultSettings.fontSize;
			}
			...
			if (params.shape && params.shape === 'round') {
				_style = 'border-radius: ' + params.radius + 'px; ' + _style;
			}
